...
renderer.view.id = "Invisible Chaos";
...
function positionParticles() {
  ...
  context.fillText(options.keyword, 0, 220);
...
function positionText() {
  ...
  context.fillText("Invisible Chaos", 0, 100);
...
function particle(text) {
  ...
  var radius;
  $this.radius = radius = $this.text ? Math.random() * 3.5 : Math.random() * 10;
  ...
  // $this.timer = Math2.randomInt(0, 200);
  $this.timer = 200;
  ...
  $this.setPosition = function(x, y) {
    if ($this.text) {
      $this.x = x + (options.width / 2 - 160);
      $this.y = y + (options.height / 2 + 20);
    } else {
      $this.x = x + (options.width / 2 - 250);
      $this.y = y + (options.height / 2 - 255);
    }
  };
...
//if false, the particles will fly
var check=true;
...
function update() {
  ...
  for (i = 0; i < particles.length; i++) {
    ...
    p.scale.x = p.scale.y = scale = Math.max(Math.min(2.5 - (Math2.distance(p.x, p.y, mousePos.x, mousePos.y) / 150), 150), 1.2);
